...
export default function SellerRegister({ onNavigate }: { onNavigate: (id: string) => void }) {
  ...
  // ── Verification & Backend Connection ──────────────────────────────────────
  const handleVerificationSubmit = async () => {
    ...
    try {
      // Hit the Spring Boot endpoint to create a Seller
      ...
      if (response.ok) {
        ...
        // Store the real Java database ID so you can link items to this seller later
        localStorage.setItem('seller_db_id', data.id);
      } else {
        console.error("Failed to register seller to backend");
      }
    } catch (error) {
      console.error("Backend not connected, proceeding with local state.", error);
    }

    // Keep existing local storage sync
    const accounts = getAccounts();
              ...
              <CardContent className="p-8">
                ...
                <div className="flex flex-col sm:flex-row gap-4 mt-12 pt-8 border-t">
                  ...
                  <Button
                      ...
                      className="flex-[2] h-12 font-black uppercase text-sm tracking-widest order-1 sm:order-2 shadow-xl shadow-primary/20 disabled:opacity-50"
                      onClick={handleVerificationSubmit}
                  >
                    {isSubmitting ? "Syncing..." : "Submit & Go to Dashboard"}
                  </Button>
                </div>
              </CardContent>
            </Card>
