...
import { Sparkles, Clock, Zap, BrainCircuit } from "lucide-react";

export default function ProductivityGapCard() {
  const [suggestion, setSuggestion] = useState<{ message: string; type: 'gap' | 'busy' } | null>(null);
  ...
  useEffect(() => {
    const analyzeSchedule = () => {
      ...
      events.filter((e: any) => e.date === todayStr).forEach((e: any) => {
        busyBlocks.push({
      ...
      todos.filter((t: any) => t.dueDate === todayStr && t.dueTime && !t.completed).forEach((t: any) => {
        const start = timeToMinutes(t.dueTime);
        busyBlocks.push({ start, end: start + 45, title: t.title });
      });
    ...
    analyzeSchedule();
    window.addEventListener("local-storage-update", analyzeSchedule);
    return () => window.removeEventListener("local-storage-update", analyzeSchedule);
  }, []);

  const timeToMinutes = (t: string) => t.split(':').reduce((h, m) => +h * 60 + +m, 0);
