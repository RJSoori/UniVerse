...
export function Marketplace({ onNavigate }: MarketplaceProps) {
  ...
  const handleReport = () => {
    ...
    const reports = JSON.parse(localStorage.getItem("universe-reports") || "[]");
    reports.push({
      listingId: selectedListingForReport.id,
      reason: reportReason,
      reportedAt: new Date().toISOString(),
    });
    localStorage.setItem("universe-reports", JSON.stringify(reports));
          ...
          <div className="bg-background rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            {selectedListing.images.length > 0 ? (
              <div className="flex gap-2 p-4 overflow-x-auto">
                {selectedListing.images.map((src, i) => (
                  <img key={i} src={src} alt="" className="h-40 w-40 object-cover rounded-xl flex-shrink-0 border border-border" />
                ))}
              </div>
            ) : (
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <Button className="flex-1" onClick={() => alert(`Contact the seller at: ${selectedListing.contactNumber}`)}>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setShowSellerPrompt(false)}>
          <div className="bg-background rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-3 pt-2">
              <Button className="w-full" onClick={() => { setShowSellerPrompt(false); onNavigate?.("seller-register"); }}>Login to Seller Account</Button>
              <Button variant="outline" className="w-full" onClick={() => { setShowSellerPrompt(false); onNavigate?.("seller-register"); }}>Create a Seller Account</Button>
            </div>
          </div>
        </div>
      )}
