...
export function JobHub({ onNavigate }: JobHubProps) {
  // Shared storage key with Recruiter Portal (used as a fallback)
  const [localJobs] = useUniStorage<any[]>("university-jobs", []);

  // --- Backend State ---
  const [jobs, setJobs] = useState<any[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  // Get student email from localStorage for applications
  const studentEmail = localStorage.getItem("university-student-email") || "";
    ...
    const fetchJobs = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Map backend data to fit your existing UI
          const formattedJobs = data.map((job: any) => ({
            ...job,
            company: job.recruiter?.companyName || "Verified Recruiter",
            contactPerson: job.recruiter?.contactPerson || "Admin",
            // UI Fallbacks for fields not currently in the Java Model
            workType: "Hybrid",
        ...
        console.error(
          "Backend fetch failed, falling back to local storage:",
          error,
        );
        setJobs(localJobs); // Fallback if backend is off
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleReport = () => {
    ...
    const reports = JSON.parse(
      localStorage.getItem("university-reports") || "[]",
    );
    reports.push({
