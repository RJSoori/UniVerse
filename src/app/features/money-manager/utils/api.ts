...
export async function saveMoneyManagerState(state: MoneyManagerRemoteState): Promise<void> {
  // Wallet ids are referenced by transactions and recurring expenses, so save
  // wallets first. The backend preserves owned/new ids on bulk replace and
  // rejects foreign ids, which keeps this dependency stable without trusting
  // arbitrary client-supplied ids.
  await requestJson<Wallet[]>("/api/money-manager/wallets/bulk", {
    method: "PUT",
