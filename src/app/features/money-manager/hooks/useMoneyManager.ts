...
import { loadMoneyManagerState, saveMoneyManagerState } from "../utils/api";
...
type MoneyManagerContextValue = ReturnType<typeof useMoneyManagerState>;

const MoneyManagerContext = createContext<MoneyManagerContextValue | null>(null);

export function MoneyManagerProvider({ children }: { children: ReactNode }) {
  const value = useMoneyManagerState();

  return createElement(MoneyManagerContext.Provider, { value }, children);
}

export function useMoneyManager() {
  const context = useContext(MoneyManagerContext);

  if (!context) {
    throw new Error("useMoneyManager must be used within MoneyManagerProvider");
  }
